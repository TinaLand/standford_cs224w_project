...
def normalize_edge_attributes(edge_weights, method='min_max', edge_type='unknown'):
    ...
    if original_max == original_min:
        ...
        print(f"      {edge_type}: All values identical ({original_min:.4f}), using uniform weights")
    ...
    if method == 'min_max':
        ...
        if std > 0:
            ...
            print(f"      {edge_type}: All values identical ({mean:.4f}), using uniform weights")
        ...
        if iqr > 0:
            ...
            # All values are identical - use uniform weights
            print(f"      {edge_type}: All values identical ({median.item():.4f}), using uniform weights")
            normalized = torch.ones_like(edge_weights)
...
def get_edge_normalization_stats():
    ...
    try:
        ...
        print(f"   Could not load correlation stats: {e}")
    ...
    try:
        ...
        print(f"   Could not load fundamental stats: {e}")
...
def load_fundamental_similarities():
    ...
    if not fund_sim_file.exists():
        print(" Fundamental similarity file not found, using empty DataFrame")
        return pd.DataFrame()
...
def load_static_connections():
    ...
    if not sector_file.exists() or not supply_comp_file.exists():
        print(" Required static files not found.")
        return None, None
...
def construct_graph_for_date(date, node_features_df, correlations_df, similarities_df, 
    ...
    if node_features is None: # <--- Handles the return from the fixed function
        print(f" No features found for date {date.strftime('%Y-%m-%d')}")
        return None
...
def main():
    ...
    try:
        ...
        print(f"\n Constructing graphs for date range:")
